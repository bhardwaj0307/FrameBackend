...
from user_2FA.tokens import get_token_generator
...
class EmailOTP(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.key:
            self.key = 1111
        return super(EmailOTP, self).save(*args, **kwargs)

    def __str__(self):
        return "Email Otp for user {user}".format(user=self.user)
...
def get_password_reset_token_expiry_time():
    ...
    return getattr(settings, 'DJANGO_REST_MULTITOKENAUTH_RESET_TOKEN_EXPIRY_TIME', 0.0833)
...
def clear_expired(expiry_time):
    ...
    """
    EmailOTP.objects.filter(created_at__lte=expiry_time).delete()
