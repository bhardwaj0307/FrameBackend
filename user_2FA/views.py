...
from .models import EmailOTP, clear_expired, get_password_reset_token_expiry_time, \
...
class ResetPasswordValidateToken(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # find token
        reset_password_token = EmailOTP.objects.filter(key=token).first()
...
class ResetPasswordConfirm(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if reset_password_token is None:
            return Response({'status': 'Incorrect OTP, Please Try again'}, status=status.HTTP_404_NOT_FOUND)
        ...
        if timezone.now() > expiry_date:
            # delete expired token
            reset_password_token.delete()
            return Response({'status': 'OTP Expired, Please Click On Resend Button'}, status=status.HTTP_404_NOT_FOUND)

        # change users password (if we got to this code it means that the user is_active)
        if reset_password_token.user.eligible_for_reset():
            # Delete all password reset tokens for this user
            EmailOTP.objects.filter(user=reset_password_token.user).delete()
...
class ResetPasswordRequestToken(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.eligible_for_reset():
            ...
            if user.email_otp.all().count() > 0:
                # yes, already has a token, re-use this token
                token = user.email_otp.all()[0]
            else:
                # no token exists, generate a new token
                token = EmailOTP.objects.create(
                    user=user,
